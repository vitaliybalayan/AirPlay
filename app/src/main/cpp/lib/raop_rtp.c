...
static int
raop_rtp_copy_remote(raop_rtp_t *raop_rtp, const struct sockaddr_storage *remote_saddr, socklen_t remote_saddr_len)
{
    assert(raop_rtp);
    assert(remote_saddr);
    if (remote_saddr_len > sizeof(raop_rtp->remote_saddr)) {
        return -1;
    }
    memset(&raop_rtp->remote_saddr, 0, sizeof(raop_rtp->remote_saddr));
    memcpy(&raop_rtp->remote_saddr, remote_saddr, remote_saddr_len);
    raop_rtp->remote_saddr_len = remote_saddr_len;
    return 0;
}

raop_rtp_t *
raop_rtp_init(logger_t *logger,
               raop_callbacks_t *callbacks,
               ...
               const unsigned char *aeskey, const unsigned char *aesiv, const unsigned char *ecdh_secret, unsigned short timing_rport)
{
    ...
    if (raop_rtp_copy_remote(raop_rtp, remote_saddr, remote_saddr_len) < 0) {
		free(raop_rtp);
		return NULL;
	}
...
static void
raop_rtp_set_port(struct sockaddr_storage *saddr, unsigned short port)
{
...
raop_rtp_thread_time(void *arg)
{
    ...
    while (1) {
        ...
        raop_rtp_set_port(&raop_rtp->remote_saddr, raop_rtp->timing_rport);
        ...
        if (sendlen <= 0) {
            logger_log(raop_rtp->logger, LOGGER_WARNING, "Audio timing send failed: %d", sendlen);
        }
        ...
        if (packetlen <= 0) {
            logger_log(raop_rtp->logger, LOGGER_WARNING, "Audio timing receive failed: %u", packetlen);
            continue;
        }
...
float
raop_rtp_get_volume(raop_rtp_t *raop_rtp)
{
    float volume;
