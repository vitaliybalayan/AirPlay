...
static int
raop_rtp_copy_remote(raop_rtp_mirror_t *raop_rtp_mirror, const struct sockaddr_storage *remote_saddr, socklen_t remote_saddr_len)
{
    assert(raop_rtp_mirror);
    assert(remote_saddr);
    if (remote_saddr_len > sizeof(raop_rtp_mirror->remote_saddr)) {
        return -1;
    }
    memset(&raop_rtp_mirror->remote_saddr, 0, sizeof(raop_rtp_mirror->remote_saddr));
    memcpy(&raop_rtp_mirror->remote_saddr, remote_saddr, remote_saddr_len);
    raop_rtp_mirror->remote_saddr_len = remote_saddr_len;
    return 0;
}

#define NO_FLUSH (-42)
raop_rtp_mirror_t *raop_rtp_mirror_init(logger_t *logger,
                                        raop_callbacks_t *callbacks,
                                        ...
                                        const unsigned char *aeskey, const unsigned char *ecdh_secret, unsigned short timing_rport)
{
    ...
    if (raop_rtp_copy_remote(raop_rtp_mirror, remote_saddr, remote_saddr_len) < 0) {
        free(raop_rtp_mirror);
        return NULL;
    }
...
static void
raop_rtp_mirror_set_port(struct sockaddr_storage *saddr, unsigned short port)
{
...
raop_rtp_mirror_thread_time(void *arg)
{
    ...
    while (1) {
        ...
        raop_rtp_mirror_set_port(&raop_rtp_mirror->remote_saddr, raop_rtp_mirror->mirror_timing_rport);
        ...
        if (sendlen <= 0) {
            logger_log(raop_rtp_mirror->logger, LOGGER_WARNING, "Mirror timing send failed: %d", sendlen);
        }
        ...
        if (packetlen <= 0) {
            logger_log(raop_rtp_mirror->logger, LOGGER_WARNING, "Mirror timing receive failed: %u", packetlen);
            continue;
        }
...
raop_rtp_mirror_thread(void *arg)
{
    ...
    while (1) {
        ...
        if (stream_fd == -1 && FD_ISSET(raop_rtp_mirror->mirror_data_sock, &rfds)) {
            ...
            logger_log(raop_rtp_mirror->logger, LOGGER_INFO, "Mirror TCP client connected");
        }
        if (stream_fd != -1 && FD_ISSET(stream_fd, &rfds)) {
            ...
            if ((packet[0] == 80 && packet[1] == 79 && packet[2] == 83 && packet[3] == 84) || (packet[0] == 71 && packet[1] == 69 && packet[2] == 84)) {
                ...
                if (payloadtype == 0) {
                    ...
                    logger_log(raop_rtp_mirror->logger, LOGGER_INFO, "Mirror video config source=%.0fx%.0f display=%.0fx%.0f", mWidthSource, mHeightSource, mWidth, mHeight);
                    /*int mRotateMode = 0;
                    ...
                    logger_log(raop_rtp_mirror->logger, LOGGER_INFO, "Mirror codec config SPS=%d PPS=%d", h264.lengthofSPS, h264.lengthofPPS);
